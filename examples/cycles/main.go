...
func main() {
	...
	p, err := cycles.NewProject(cycles.CYCLES)
	if err != nil {
		panic(err)
	}
	defer p.Close()
...
func compo(allVoices map[int][]pattern.Pattern, p midicom.MidiCom) {
	var riffPart1 []pattern.Degree = []pattern.Degree{
		0,
		-pattern.Perfect4th,
		-pattern.Perfect5th,
		pattern.Major6th,
	...
	var riffAdd []pattern.Degree
	riffAdd = append(riffAdd, riffPart1...)
	...
	pat.Notes[0].Note += midicom.Note(riffAdd[0%len(riffAdd)])
	...
	pat.Notes[0].Note += midicom.Note(riffAdd[1%len(riffAdd)])
	...
	pat.Notes[0].Note += midicom.Note(riffAdd[2%len(riffAdd)])
	...
	pat.Notes[0].Note += midicom.Note(riffAdd[3%len(riffAdd)])
	...
	pat.Notes[0].Note += midicom.Note(riffAdd[3%len(riffAdd)]) - midicom.Note(pattern.Octave*2)
	pat.Notes[0].Duration = 3000

	pat.Notes[0].CC = cycles.PT5()
	pat.Notes[0].CC[cycles.REVERB] = int8(120)
	...
	chords := []cycles.Chords{
		cycles.Major,
		cycles.Major6,
		cycles.MajorMajor7,
		cycles.MajorMajor9no5,
	}

	// Chord
	for o := range 100 {
		chordNote := baseNote()
		chordNote.Note += midicom.Note(riffAdd[o%len(riffAdd)])
		pat = pattern.Pattern{
			Midicom: p,
		...
		pat.Notes[0].Duration = pat.Notes[0].Duration + float64(o*2)

		pat.Notes[0].CC = cycles.PT6()
		pat.Notes[0].CC[cycles.SWEEP] = int8(o) % 127
		pat.Notes[0].CC[cycles.CONTOUR] = int8(o) % 127
		pat.Notes[0].CC[cycles.REVERB] += int8(o) % 110
		pat.Notes[0].CC[cycles.DECAY] = 35
		pat.Notes[0].CC[cycles.PUNCH] = 1
		pat.Notes[0].CC[cycles.REVERBSIZE] += int8(o) % 100
		pat.Notes[0].CC[cycles.REVERBTONE] += int8(o) % 120
		pat.Notes[0].CC[cycles.SHAPE] = int8(chords[o%len(chords)])
